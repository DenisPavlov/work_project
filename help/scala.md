...
#### Исключения
- кинуть исключение - <code>throw new NullPointerException("greeting was null")</code>

#### Встроенные структуры управления
- структура управления - match ???
- выражение if возвращает значение <code>val filename = if (!args.isEmpty) args(0) else "default.txt"
