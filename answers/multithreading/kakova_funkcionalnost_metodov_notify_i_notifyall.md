Метод <code>notify()</code> пробуждает один из потоков, который вызвал метод <code>wait()</code> у этого монитора. Метод <code>notifyAll</code> пробуждает все потоки. 
Очередность выполнения в этом случае будет определяться приоритетом потока.
